...
object Assignment3_1 {
  ...
  def main(args: Array[String]) {
    ...
    // SQL syntax
    val topMovies = spark.sql("SELECT movieID, AVG(rating), COUNT(movieID) FROM movie GROUP BY (movieID) HAVING COUNT(movieID) >= 100 ").orderBy(("AVG(rating)"))

    val results = topMovies.collect()
    val names = loadMovieNames()
    println
    val file = "Assignment3_1.txt"
    val writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file)))
    for (result <- results) {
      // result is just a Row at this point; we need to cast it back.
      // Each row has movieID, count as above.
      writer.write(names(result(0).asInstanceOf[Int]) + ": " + result(1) + "," + result(2) + "\n")
    }
    writer.close()
    spark.stop()
  }
